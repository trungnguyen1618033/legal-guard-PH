...
from legalguard.domain.regulatory import format_impact_alert, parse_basis_file, scan_cases


def test_parse_basis_file():
    assert parse_basis_file("nd_123_2020_hoa_don.md#Điều 9: nội dung…") == "nd_123_2020_hoa_don.md"
    assert parse_basis_file("file.md#Điều 5") == "file.md"
    assert parse_basis_file("không có dấu thăng") == ""
    assert parse_basis_file("") == ""
...
def test_scan_cases_matches_risk_and_fallback():
    affected = {"nd_123_2020_hoa_don.md": "amends"}
    case = _case(
...
def test_affected_doc_files_live_kb():
    # NĐ 70/2025 sửa NĐ 123/2020 → file 123 bị tác động (quan hệ amends).
    aff = affected_doc_files("knowledge_base", "VN", "70/2025/NĐ-CP")
    assert aff.get("nd_123_2020_hoa_don.md") == "amends"
