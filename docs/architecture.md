...
Mục tiêu: tách **nghiệp vụ (domain)** khỏi **hạ tầng (FastAPI, Qwen, Gemini, file KB)**
...
```
        ...
        │ adapters/outbound/gemini.py    │  │   knowledge_base.py            │
        │  (LLMPort)                     │  │   document_parser.py           │
        └────────────────────────────────┘  └───────────────────────────────┘
...
legalguard/
  domain/                           ← BÊN TRONG hexagon (không phụ thuộc hạ tầng)
    ...
    evidence.py                     EvidenceService (use-case doanh thu/evidence XPRIZE)
  ...
  adapters/
    ...
    outbound/qwen.py                LLMPort: Qwen Cloud (chat/complete/embed)
    outbound/gemini.py              LLMPort: Gemini (tóm tắt — ràng buộc XPRIZE)
...
|---|---|---|
| `LLMPort` | `QwenAdapter`, `GeminiAdapter`, (fake trong test) | Gọi LLM (chat/complete/embed) |
...
4. Mỗi bước ghi `TraceStep` → execution trace (debug + evidence AI-Native cho XPRIZE).
...
**(verification ∥ tóm tắt Gemini ∥ gắn căn cứ pháp lý) chạy SONG SONG** → lưu case → trả `AnalysisResult`.
...
- **Testability:** test inject fake `LLMPort`/retriever → chạy offline, không tốn quota
  (xem `tests/`). Đây là lý do 228 test chạy được mà không gọi Qwen/Gemini thật.
- **Ràng buộc 2 cuộc thi:** Qwen (reasoner `qwen3.7-max` + judge nhanh `qwen-flash`) và Gemini
  (summarizer) chỉ là các adapter của cùng `LLMPort` → thỏa cả hai + right-sizing latency mà không
  phân nhánh trong domain (domain chỉ thấy `reasoner`/`judge`/`summarizer`).
...
## 6. Giai đoạn sau (XPRIZE) — vẫn trong khuôn Hexagonal
