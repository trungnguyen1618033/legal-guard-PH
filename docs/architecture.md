...
**verification pass** (LLM-as-judge kiểm grounding) → tóm tắt bằng Gemini (≥1 call) →
lưu case → trả `AnalysisResult`. Lỗi provider → `LLMError` đã làm sạch → degrade, không crash.
...
- **Testability:** test inject fake `LLMPort`/retriever → chạy offline, không tốn quota
  (xem `tests/`). Đây là lý do 23 test chạy được mà không gọi Qwen/Gemini thật.
- **Ràng buộc 2 cuộc thi:** Qwen (reasoner) và Gemini (summarizer) chỉ là 2 adapter của
  cùng `LLMPort` → thỏa cả hai mà không phân nhánh trong domain.
