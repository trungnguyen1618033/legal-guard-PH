...
_FAST_MAX_WINDOWS = 40  # trần cửa sổ fast (~480k ký tự) — HĐ khổng lồ → cắt + đánh dấu truncated (chống chi
#                        phí/latency phình; realistic HĐ <30 cửa sổ. HĐ lớn cực đoan nên dùng deep/tách file)
    ...
    def analyze(self, contract_text: str, org: Organization, lang: str = "en",
        ...
        if mode == "fast":
            ...
            if text_chars <= _FAST_MAX:               # HĐ ngắn/vừa → 1 call
                ...
                if len(windows) > _FAST_MAX_WINDOWS:  # HĐ khổng lồ → cắt + truncated (note 'phần cuối chưa rà')
                    windows, truncated = windows[:_FAST_MAX_WINDOWS], True
