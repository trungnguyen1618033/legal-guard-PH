...
(verify chữ ký trên raw body, replace_original xác nhận). **Reply phân tích (có case_id) thêm nút GHI KẾT QUẢ
đàm phán** ✓ Chốt/~ Một phần/✗ Không đạt (`_outcome_blocks`/`_OC_RESULT`) → `_record_deal_outcome` ghi Outcome
cho MỌI điều khoản của case → nuôi win-rate (flywheel ngay trên Slack, cô lập org). Lookup chat hiện cả nguồn
