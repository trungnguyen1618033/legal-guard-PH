...
outcome=accepted + feedback=helpful (`_RV_ACTION`; map `_OC_RESULT` giữ cho tin oc_* CŨ).
**Sửa lại = TINH CHỈNH per-clause (KHÔNG xoá bài)** (feedback user 16/7: 'Sửa lại' cũ replace_original ghi
đè cả tin → MẤT bài phân tích): bấm → `conv.pending_edit=case_id` + hỏi 'muốn sửa điều/ý nào?' (KHÔNG
